...
import wandb
from  dataset import get_dataloader, short_class_names
...
def test(model, data_loader, validation=True):
    ...
    if validation:
        ...
        wandb.log({'test_acc': acc, 'test_loss': loss, "test_f1": f1})
...
if __name__ == "__main__":
    ...
    for split_num in range(config["splits"]):
        ...
        for epoch in tqdm(range(config['num_epochs'])):
            ...
            train_loss, train_acc, train_f1 = train(tagging_model, train_loader)
            log = f'訓練集 Loss: {train_loss:.4f} Accuracy: {train_acc:.2f} F1 score: {train_f1:.2f}\n'
            print(log)
            # with open(log_file, 'a') as file:
            #     file.write(log)
            val_loss, val_acc, val_f1 = test(tagging_model, valid_loader)
            log = f'驗證集 Loss: {val_loss:.4f} Accuracy: {val_acc:.2f} F1 score: {val_f1:.2f}\n'
            print(log)
            # with open(log_file, 'a') as file:
            #     file.write(log)
            state_dict_path = exp_dir / f"Epoch_{epoch}_loss_{val_loss:.3f}_f1_{val_f1:.3f}.pt"
            torch.save(tagging_model.state_dict(), state_dict_path)
            if epoch == 0 or best_f1 < val_f1:
        ...
        print("Testing")
        test_loss, test_acc, test_f1 = test(tagging_model,
                                            test_loader,
                                            validation=False)
        log = f'測試集 Loss: {test_loss:.4f} Accuracy: {test_acc:.2f} F1 score: {test_f1:.2f}\n'
        # print(log)
        # with open(log_file, 'a') as file:
        #     file.write(log)
        wandb.finish()
